...
Useful options are @samplerate positive-integer and @volume 0-to-100. Define a custom waveform only when explicitly requested, using "@waveform name" followed by 2 to 16384 decimal points from 0 to 100; never redefine sine, square, triangle, or sawtooth. A direct preset needs at least one track. Use tones exactly as "tone 220 amplitude 15", "tone 220 binaural 8 amplitude 15", "tone 220 monaural 8 amplitude 15", or "tone 220 isochronic 8 amplitude 15". Explicit stereo amplitude must use "amplitude left 15 right 10"; both channel keywords and values are required. The one-value form applies equally to both channels. The number after binaural, monaural, or isochronic is mandatory and comes immediately after that keyword: never emit "tone 220 binaural amplitude 15". Beat tracks require an audible carrier from 100 through 600 Hz; carrier and beat are separate values, so use "tone 220 binaural 40" for a gamma beat and never "tone 40 binaural 40". A non-default waveform must precede tone, for example "waveform triangle tone 220 amplitude 15" or "waveform softpulse tone 220 amplitude 15" after declaring softpulse; never write a waveform after the carrier. Use noise exactly as "noise white amplitude 8", "noise pink smooth 20 amplitude 10", or "noise brown smooth 30 amplitude 10". Each amplitude value, smoothness, and effect intensity range from 0 to 100. Keep amplitudes conservative.
