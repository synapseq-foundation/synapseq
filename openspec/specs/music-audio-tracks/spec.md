## Purpose
Define registration, parsing, loading, rendering, effects, crossfade, and size-limit behavior for finite music audio tracks.
...
#### Scenario: Same music source starts independently on another channel
- **WHEN** a music source reaches EOF on one channel and the same music source is later used on another channel
- **THEN** the later channel plays that music source from the beginning while the ended channel remains silent
